...
- **Message verification** — Multi-factor matching (9 confidence levels) and exception handling implemented
...
| Generic IMAP (Gmail/Workspace) | Generic IMAP (Gmail/Workspace) | imapsync | IMAPS with app passwords / XOAUTH2 | Gmail labels → IMAP folders; [Gmail]/ prefix | Google Workspace 2024+, imapsync 2.314 | Code path verified | Code path verified + integration tests | Recovery logic tested | Message verification + confidence levels implemented | See PROVIDER_SETUP.md and OAUTH_SETUP.md for setup. Multi-factor message matching validates correctness. Provider throttling: 100 msgs/sec. Known: "All Mail" duplication expected behavior. |
| Generic IMAP (Microsoft 365) | Generic IMAP (Microsoft 365) | imapsync | IMAPS with app passwords / OAuth2 | Flat; special folders prefixed (Deleted Items, Sent Items, etc.) | Microsoft 365 2024+, imapsync 2.314 | Code path verified | Code path verified + integration tests | Recovery logic tested | Message verification + confidence levels implemented | See PROVIDER_SETUP.md and OAUTH_SETUP.md for setup. Error classification and throttling (150 msgs/sec) built-in. Soft throttling handling documented. Requires quota verification. |
| Generic IMAP (Fastmail) | Generic IMAP (Fastmail) | imapsync | IMAPS with app passwords | JMAP-first IMAP; `/` separators; full SPECIAL-USE | Fastmail 2024+, imapsync 2.314 | Code path verified | Code path verified + integration tests | Recovery logic tested | Message verification + confidence levels implemented | See PROVIDER_SETUP.md for setup. Conservative throttling (50 msgs/sec) default. Full SPECIAL-USE support expected. Pre-migration risk assessment available. |
