...
MailSwiftSync can import a migration list from CSV, XLS, or XLSX and process rows with a bounded worker pool. Choose 1–16 concurrent workers to balance migration-window speed against provider throttling. Every mailbox must first complete a matching dry validation; after that, the same durable queue can be explicitly promoted to a live batch run. Live retries exclude rows already marked **Verified** by default, so a repaired batch runs only unresolved mailboxes. Use **Include already verified mailboxes (explicit re-run)** only when deliberately repeating an evidence-backed migration.
