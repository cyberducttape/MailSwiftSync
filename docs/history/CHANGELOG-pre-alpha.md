...
Archived pre-alpha development history for MailSwiftSync. This file is kept
for traceability and is not the current release changelog; some entries below
describe superseded implementation stages. See the root `CHANGELOG.md` for
the supported release history and current product boundaries.
...
- Removed dormant remote-Dovecot SSH configuration and command generation;
  ...
  exists, and legacy profiles requesting SSH now fail closed when loaded.
...
- Applied the private-directory trust boundary to atomic report/artifact
  ...
  output directories from becoming privileged write targets; proof/report test
  fixtures now create their temporary parents through the same policy.
...
- Restored a warning-clean release build: removed the obsolete dead-code
  ...
  explicitly, and the parent-permission regression fixture uses a safe,
  non-writable `0750` mode, making the hardened pathname boundary portable
  across CI hosts.
...
- Moved the live batch confirmation, including durable mailbox-state checks,
  into the same dedicated safety-dialog module.
...
- Made the product IMAP lab reject non-packaged imapsync versions explicitly,
  keeping its verification grammar tied to the distributed 2.314 engine.
...
- Extracted durable event recording and active-run event batching into
  `core/events.rs`, keeping event transaction, detail-boundary, and private
  event insertion logic separate from mailbox/run state transitions.
...
- Local Dovecot commands and verification now receive source credentials through a child environment variable and Dovecot `$ENV:` expansion; the former remote SSH path was later removed.
...
- Historical security note: an earlier remote-Dovecot compatibility path used a destination-side `imapc_password` override and could expose credentials through process inspection. That path was subsequently removed; current builds support local Dovecot execution only.
