...
The evidence schema can hold source/destination folder and message counts, byte counts, an optional unresolved-message count, and failed messages. Reports expose named assurance levels: Level 0 (process completed, verification incomplete), Level 1 (engine completion evidence), Level 2 (aggregate folder/message/size reconciliation), Level 3 (metadata-level message reconciliation), and Level 4 (full body-content reconciliation). Encrypted imapsync live runs now perform bounded Level 3-style reconciliation using independently fetched Message-ID, INTERNALDATE, and RFC822.SIZE values; native Dovecot runs remain at Level 2. UID enumeration uses bounded `UID SEARCH` windows derived from `UIDNEXT`, so a mailbox-wide `UID SEARCH ALL` response is never materialized. The verifier still materializes both account maps and reconciliation indexes, however, and therefore remains capped at one million records, a separate estimated 64 MiB mismatch-detail budget, and an aggregate estimated-state admission budget that includes reconciliation index and classification overhead; those estimates do not guarantee peak process memory. SQLite-backed streaming reconciliation is still required for very large MSP migrations, alongside body hashing, UIDVALIDITY-aware checkpoints, and Level 4 content proof. The compatibility percentage is not a probability: exact engine-confirmed evidence may reach 100%, exact aggregate evidence is capped at 85%, and aggregate mismatches receive 0 rather than a reassuring partial score. When the imapsync completion proof is absent, `unmatched_messages` is `null` rather than a fake count; the UI and exports must report the count as unknown rather than infer success from process exit status.

The ledger records project lifecycle, structured run events, reconciliation evidence, and bounded per-message mismatch rows; verbose engine transcripts remain bounded process-local diagnostics and are not persisted. The Dovecot adapter obtains aggregate folder/message/virtual-size status from both the remote `imapc` source and the destination after a live run. Native Dovecot strategy selection is durable in the immutable run snapshot: backup is used for initial/incremental mirrors and `sync -1` for preservation passes; exit code 2 remains delta-required until a later pass returns 0. The imapsync adapter consumes its final summary and, for encrypted live runs, independently fetches bounded Message-ID, INTERNALDATE, and RFC822.SIZE metadata from every selectable folder before committing verification evidence and mismatch details atomically. The in-memory mismatch-detail budget is estimated at 64 MiB; exceeding it fails closed for operator review. Verification exports include the durable run identifier and timestamps, so an audit artifact can be traced to one execution. Unix migrations run in their own process group; cancellation allows graceful shutdown before escalation, and Linux children receive a parent-death signal as an additional crash-safety measure. The runner records each started process together with platform process start-time, process-group, and session identity. Startup terminates a recorded Unix group only when those identity values still match; otherwise it refuses to signal the PID and leaves the job for operator review. Windows children are attached to a kill-on-close Job Object; macOS validates identity through `proc_pidinfo` and retains the conservative no-signal fallback when that native query fails. The process record is best-effort during the short interval before the UI receives the child-start event. The current Dovecot checkpoint is an engine resume token, not UIDVALIDITY-aware verification evidence; content hashes and UIDVALIDITY-aware checkpoints remain future milestones.
...
and content hashes remain future milestones.
