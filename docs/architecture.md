...
Profile replacement is flushed before rename, and SQLite uses WAL mode, foreign keys, and a bounded busy timeout. The persistent database directory is owner-only on Unix so WAL/SHM sidecars remain contained even when SQLite creates them after startup. Evidence history is transactionally written and must reference an existing run (except for the explicitly supported legacy import path).
