...
- Native Dovecot runs provide aggregate evidence, while encrypted imapsync runs also perform bounded metadata-level message reconciliation (folder, Message-ID, INTERNALDATE, and RFC822.SIZE). This is not body-content proof, and UIDVALIDITY-aware evidence is not implemented. Dovecot live runs persist and reuse engine-emitted `-s` state strings as a resumability optimization, atomically with each child result.
