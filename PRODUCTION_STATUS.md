...
- Required documentation files and selected safety-critical wording
- Provider/authentication guidance smoke checks
- These tests do not prove live provider evidence or controller wiring
...
Test counts and pass/fail status are generated by CI rather than maintained in
this document. The local command remains `cargo test --locked --all-targets
--all-features`.
...
tests; message-level verification is not wired into those labs yet.
